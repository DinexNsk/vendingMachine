...
export const Output = ({
    ...
    const showChangeText = useCallback(() => {
        if (!moneyChange) {
            return 'No change!'
        }
        return Object.keys(changeResult).sort((a, b) => Number(b) - Number(a)).map(el =>
            <div key={`changeId-${el}`} style={{ marginBottom: 10 }}>
                {`${el} R: ${changeResult[el]} coins`}
            </div>);
    }, [changeResult, moneyChange])

    const displayChange = boughtItem ? showChangeText() : '';
